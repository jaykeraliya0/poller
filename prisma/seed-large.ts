...
async function main() {
  ...
  await db.$executeRawUnsafe('TRUNCATE TABLE "answers", "responses", "poll_options", "polls", "users", "email_tokens" RESTART IDENTITY CASCADE');
  ...
  const [users, polls, responses, answers, groups] = await Promise.all([
    db.user.count(),
    db.poll.count(),
  ...
  console.log(
    `Generated ${stats.polls} polls (${stats.privatePolls} private), ${stats.groups} groups, ${stats.invites} invites and group memberships, ${stats.responses} responses, ${stats.answers} answers in ${((Date.now() - started) / 1000).toFixed(1)}s`,
  );
  console.log(`Database now has ${users} users (all with confirmed emails), ${polls} polls, ${groups} groups, ${responses} responses, ${answers} answers`);
